...
exports.auth = function(req, res) {
  ...
  User.findOne({email}, function(err, user) {
    ...
    if (user.hasSamePassword(password)) {
      ...
      return res.status(422).send({errors: [{title: 'Wrong Data', details: 'Wrong email and password!'}]});
    }
  });
}
...
exports.register = function(req, res) {
  ...
  if (password !== passwordConfirmation) {
    return res.status(422).send({errors: [{title: 'Invalid password', details: 'Password is not a same as confirmation!'}]});
  }
